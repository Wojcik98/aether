...
TEST(aether_app, localization_test_dummy) {
    MapMock map;
    MapConfident<MapMock> map_confident(map);

    // corners are always obstacles
    ASSERT_EQ(map_confident.is_obstacle(0.0f, 0.0f), true);

    // middle of the cell is never an obstacle
    ASSERT_EQ(map_confident.is_obstacle(CELL_SIZE / 2.0f, CELL_SIZE / 2.0f),
              false);
}
