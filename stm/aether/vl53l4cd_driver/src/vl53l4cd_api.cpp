...
static const uint8_t VL53L4CD_DEFAULT_CONFIGURATION[] = {
    ...
    (pull up at AVDD) */
    0x11, /* 0x30 : set bit 4 to 0 for active high interrupt and 1 for active
    ...
    high, 2-> Out of window, 3->In window, 0x20-> New sample ready , TBC */
...
VL53L4CD_Error VL53L4CD_API::get_sw_version(VL53L4CD_Version_t *p_Version) {
...
VL53L4CD_Error VL53L4CD_API::set_i2c_address(uint8_t new_address) {
    VL53L4CD_Error status = VL53L4CD_ERROR_NONE;

    status |= i2c->write_8bit(VL53L4CD_I2C_SLAVE__DEVICE_ADDRESS,
                              (uint8_t)(new_address >> (uint8_t)1));
    return status;
}

VL53L4CD_Error VL53L4CD_API::get_sensor_id(uint16_t *p_id) {
    VL53L4CD_Error status = VL53L4CD_ERROR_NONE;

    status |= i2c->read_16bit(VL53L4CD_IDENTIFICATION__MODEL_ID, p_id);
    return status;
}

VL53L4CD_Error VL53L4CD_API::sensor_init() {
    VL53L4CD_Error status = VL53L4CD_ERROR_NONE;
    uint8_t Addr, tmp;
    ...
    do {
        status |= i2c->read_8bit(VL53L4CD_FIRMWARE__SYSTEM_STATUS, &tmp);
    ...
    for (Addr = (uint8_t)0x2D; Addr <= (uint8_t)0x87; Addr++) {
        status |= i2c->write_8bit(
            Addr, VL53L4CD_DEFAULT_CONFIGURATION[Addr - (uint8_t)0x2D]);
    }

    /* Start VHV */
    status |= i2c->write_8bit(VL53L4CD_SYSTEM_START, (uint8_t)0x40);
    i = (uint8_t)0;
    continue_loop = (uint8_t)1;
    do {
        status |= check_for_data_ready(&tmp);
    ...
    status |= clear_interrupt();
    status |= stop_ranging();
    status |= i2c->write_8bit(VL53L4CD_VHV_CONFIG__TIMEOUT_MACROP_LOOP_BOUND,
                              (uint8_t)0x09);
    status |= i2c->write_8bit(0x0B, (uint8_t)0);
    status |= i2c->write_16bit(0x0024, 0x500);

    status |= set_range_timing(50, 0);

    return status;
}

VL53L4CD_Error VL53L4CD_API::clear_interrupt() {
    VL53L4CD_Error status = VL53L4CD_ERROR_NONE;

    status |= i2c->write_8bit(VL53L4CD_SYSTEM__INTERRUPT_CLEAR, 0x01);
    return status;
}

VL53L4CD_Error VL53L4CD_API::start_ranging() {
    VL53L4CD_Error status = VL53L4CD_ERROR_NONE;
    uint32_t tmp;

    status |= i2c->read_32bit(VL53L4CD_INTERMEASUREMENT_MS, &tmp);

    /* Sensor runs in continuous mode */
    if (tmp == (uint32_t)0) {
        status |= i2c->write_8bit(VL53L4CD_SYSTEM_START, 0x21);
    }
    /* Sensor runs in autonomous mode */
    else {
        status |= i2c->write_8bit(VL53L4CD_SYSTEM_START, 0x40);
    }

    return status;
}

VL53L4CD_Error VL53L4CD_API::stop_ranging() {
    VL53L4CD_Error status = VL53L4CD_ERROR_NONE;

    status |= i2c->write_8bit(VL53L4CD_SYSTEM_START, 0x80);
    return status;
}

VL53L4CD_Error VL53L4CD_API::check_for_data_ready(uint8_t *p_is_data_ready) {
    VL53L4CD_Error status = VL53L4CD_ERROR_NONE;
    uint8_t temp;
    uint8_t int_pol;

    status |= i2c->read_8bit(VL53L4CD_GPIO_HV_MUX__CTRL, &temp);
    ...
    status |= i2c->read_8bit(VL53L4CD_GPIO__TIO_HV_STATUS, &temp);
...
VL53L4CD_Error VL53L4CD_API::set_range_timing(uint32_t timing_budget_ms,
    ...
    if (status != (uint8_t)VL53L4CD_ERROR_INVALID_ARGUMENT) {
        ...
        ls_byte =
            ((timing_budget_us + ((tmp >> 6) >> 1)) / (tmp >> 6)) - (uint32_t)1;
        ...
        ms_byte =
            (uint16_t)(ms_byte << 8) + (uint16_t)(ls_byte & (uint32_t)0xFF);
        status |= i2c->write_16bit(VL53L4CD_RANGE_CONFIG_A, ms_byte);
        ...
        ms_byte =
            (uint16_t)(ms_byte << 8) + (uint16_t)(ls_byte & (uint32_t)0xFF);
        status |= i2c->write_16bit(VL53L4CD_RANGE_CONFIG_B, ms_byte);
    }
...
VL53L4CD_Error
VL53L4CD_API::get_range_timing(uint32_t *p_timing_budget_ms,
    ...
    status |= i2c->read_32bit(VL53L4CD_INTERMEASUREMENT_MS, &tmp);
    status |= i2c->read_16bit(VL53L4CD_RESULT__OSC_CALIBRATE_VAL, &clock_pll);
    clock_pll = clock_pll & (uint16_t)0x3FF;
    ...
    *p_inter_measurement_ms = (uint16_t)(tmp / (uint32_t)clock_pll);
    ...
    status |= i2c->read_16bit(0x0006, &osc_frequency);
    status |=
        i2c->read_16bit(VL53L4CD_RANGE_CONFIG_A, &range_config_macrop_high);
    ...
    *p_timing_budget_ms = (((ls_byte + (uint32_t)1) * (macro_period_us >> 6)) -
    ...
    if (ms_byte < (uint8_t)12) {
        *p_timing_budget_ms =
            (uint32_t)(*p_timing_budget_ms >> (uint8_t)ms_byte);
    }
...
VL53L4CD_Error VL53L4CD_API::get_result(VL53L4CD_ResultsData_t *p_result) {
    VL53L4CD_Error status = VL53L4CD_ERROR_NONE;
    uint16_t temp_16;
    uint8_t temp_8;
    uint8_t status_rtn[24] = {255, 255, 255, 5,   2,   4,   1,   7,
                              3,   0,   255, 255, 9,   13,  255, 255,
                              255, 255, 10,  6,   255, 255, 11,  12};

    status |= i2c->read_8bit(VL53L4CD_RESULT__RANGE_STATUS, &temp_8);
    ...
    status |= i2c->read_16bit(VL53L4CD_RESULT__SPAD_NB, &temp_16);
    p_result->number_of_spad = temp_16 / (uint16_t)256;

    status |= i2c->read_16bit(VL53L4CD_RESULT__SIGNAL_RATE, &temp_16);
    p_result->signal_rate_kcps = temp_16 * (uint16_t)8;

    status |= i2c->read_16bit(VL53L4CD_RESULT__AMBIENT_RATE, &temp_16);
    p_result->ambient_rate_kcps = temp_16 * (uint16_t)8;

    status |= i2c->read_16bit(VL53L4CD_RESULT__SIGMA, &temp_16);
    p_result->sigma_mm = temp_16 / (uint16_t)4;

    status |= i2c->read_16bit(VL53L4CD_RESULT__DISTANCE, &temp_16);
...
VL53L4CD_Error VL53L4CD_API::set_offset(int16_t OffsetValueInMm) {
    VL53L4CD_Error status = VL53L4CD_ERROR_NONE;
    uint16_t temp;

    temp = (uint16_t)((uint16_t)OffsetValueInMm * (uint16_t)4);

    status |= i2c->write_16bit(VL53L4CD_RANGE_OFFSET_MM, temp);
    status |= i2c->write_16bit(VL53L4CD_INNER_OFFSET_MM, (uint8_t)0x0);
    status |= i2c->write_16bit(VL53L4CD_OUTER_OFFSET_MM, (uint8_t)0x0);
    return status;
}

VL53L4CD_Error VL53L4CD_API::get_offset(int16_t *p_offset) {
    VL53L4CD_Error status = VL53L4CD_ERROR_NONE;
    uint16_t temp;

    status |= i2c->read_16bit(VL53L4CD_RANGE_OFFSET_MM, &temp);
...
VL53L4CD_Error VL53L4CD_API::set_xtalk(uint16_t XtalkValueKcps) {
    VL53L4CD_Error status = VL53L4CD_ERROR_NONE;

    status |= i2c->write_16bit(VL53L4CD_XTALK_X_PLANE_GRADIENT_KCPS, 0x0000);
    status |= i2c->write_16bit(VL53L4CD_XTALK_Y_PLANE_GRADIENT_KCPS, 0x0000);
...
VL53L4CD_Error VL53L4CD_API::get_xtalk(uint16_t *p_xtalk_kcps) {
    VL53L4CD_Error status = VL53L4CD_ERROR_NONE;
    float_t tmp_xtalk;

    status |= i2c->read_16bit(VL53L4CD_XTALK_PLANE_OFFSET_KCPS, p_xtalk_kcps);
...
VL53L4CD_Error VL53L4CD_API::set_detection_thresholds(uint16_t distance_low_mm,
                                                      uint16_t distance_high_mm,
                                                      uint8_t window) {
    VL53L4CD_Error status = VL53L4CD_ERROR_NONE;

    status |= i2c->write_8bit(VL53L4CD_SYSTEM__INTERRUPT, window);
    status |= i2c->write_16bit(VL53L4CD_THRESH_HIGH, distance_high_mm);
    status |= i2c->write_16bit(VL53L4CD_THRESH_LOW, distance_low_mm);
    return status;
}

VL53L4CD_Error
VL53L4CD_API::get_detection_thresholds(uint16_t *p_distance_low_mm,
                                       uint16_t *p_distance_high_mm,
                                       uint8_t *p_window) {
    VL53L4CD_Error status = VL53L4CD_ERROR_NONE;

    status |= i2c->read_16bit(VL53L4CD_THRESH_HIGH, p_distance_high_mm);
    status |= i2c->read_16bit(VL53L4CD_THRESH_LOW, p_distance_low_mm);
    status |= i2c->read_8bit(VL53L4CD_SYSTEM__INTERRUPT, p_window);
    *p_window = (*p_window & (uint8_t)0x7);

    return status;
}

VL53L4CD_Error VL53L4CD_API::set_signal_threshold(uint16_t signal_kcps) {
    VL53L4CD_Error status = VL53L4CD_ERROR_NONE;

    status |= i2c->write_16bit(VL53L4CD_MIN_COUNT_RATE_RTN_LIMIT_MCPS,
                               signal_kcps >> 3);
    return status;
}

VL53L4CD_Error VL53L4CD_API::get_signal_threshold(uint16_t *p_signal_kcps) {
    VL53L4CD_Error status = VL53L4CD_ERROR_NONE;
    uint16_t tmp = 0;

    status |= i2c->read_16bit(VL53L4CD_MIN_COUNT_RATE_RTN_LIMIT_MCPS, &tmp);
    *p_signal_kcps = tmp << 3;

    return status;
}

VL53L4CD_Error VL53L4CD_API::set_sigma_threshold(uint16_t sigma_mm) {
    VL53L4CD_Error status = VL53L4CD_ERROR_NONE;

    if (sigma_mm > (uint16_t)((uint16_t)0xFFFF >> 2)) {
        status |= (uint8_t)VL53L4CD_ERROR_INVALID_ARGUMENT;
    } else {
        status |= i2c->write_16bit(VL53L4CD_RANGE_CONFIG__SIGMA_THRESH,
                                   sigma_mm << 2);
    }
...
VL53L4CD_Error VL53L4CD_API::get_sigma_threshold(uint16_t *p_sigma_mm) {
    VL53L4CD_Error status = VL53L4CD_ERROR_NONE;

    status += i2c->read_16bit(VL53L4CD_RANGE_CONFIG__SIGMA_THRESH, p_sigma_mm);
    *p_sigma_mm = *p_sigma_mm >> 2;

    return status;
}

VL53L4CD_Error VL53L4CD_API::start_temperature_update() {
    VL53L4CD_Error status = VL53L4CD_ERROR_NONE;
    ...
    status |= i2c->write_8bit(VL53L4CD_VHV_CONFIG__TIMEOUT_MACROP_LOOP_BOUND,
                              (uint8_t)0x81);
    status |= i2c->write_8bit(0x0B, (uint8_t)0x92);
    status |= i2c->write_8bit(VL53L4CD_SYSTEM_START, (uint8_t)0x40);
    ...
    status +=
        i2c->write_8bit(VL53L4CD_VHV_CONFIG__TIMEOUT_MACROP_LOOP_BOUND, 0x09);
    status += i2c->write_8bit(0x0B, 0);
    return status;
}
