...
void timer_callback(rcl_timer_t *timer, int64_t last_call_time) {
    ...
    if (timer != NULL) {
        auto result = dummy_function(msg.data, 1);
        printf("Publishing: %d\n", (int)msg.data);
...
void wait_for_agent(rmw_init_options_t *rmw_options) {
    auto result = rmw_uros_discover_agent(rmw_options);

    if (result == RMW_RET_INVALID_ARGUMENT) {
        printf("Invalid argument\n");
    }

    while (result != RMW_RET_OK) {
        printf("Waiting for agent\n");
        sleep(1);
        result = rmw_uros_discover_agent(rmw_options);
    }
    printf("Agent found\n");
}
